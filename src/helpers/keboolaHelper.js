...
export function parseConfiguration(configObject) {
  return new Promise((resolve, reject) => {
    ...
    if (!accessToken) {
      reject('Parameter #accessToken is not defined! Please check out the documentation for more information.');
    }
    const account = configObject.get('parameters:account');
    if (!account) {
      reject('Parameter account is not defined! Please check out the documentation for more information.');
    }
